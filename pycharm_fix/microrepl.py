...
STDIN_FLAGS = ENABLE_PROCESSED_INPUT | ENABLE_VIRTUAL_TERMINAL_INPUT
...
def windows_set_vt100(enable):
    ...
    if any([h[n] is None or h[n] == HANDLE(-1) or (GetFileType(h[n]) & 3) != FILE_TYPE_CHAR
            for n in stdio]):
    ...
    if enable:
        ...
        SetConsoleMode(h[stdout],
                       mode[stdout].value | ENABLE_VIRTUAL_TERMINAL_PROCESSING)  # | ENABLE_VIRTUAL_TERMINAL_INPUT)
    else:
...
def connect_miniterm(port):
    try:
        ser = serial.serial_for_url(port, BAUDRATE, parity=PARITY, rtscts=False,
                            xonxoff=False)
...
def main():
    ...
    # os.system('')   # https://stackoverflow.com/a/39675059
    port = sys.argv[1]
